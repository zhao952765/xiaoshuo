...
复制
# 1. 克隆仓库
git clone https://github.com/zhao952765/xiaoshuo.git
cd xiaoshuo
...
electron-builder	^26.8.1	NSIS / DMG / AppImage 安装包打包
...
存储位置：根状态 (emotionEvents, outlineNodes)，不嵌套在 novel 对象中，避免同步问题。
...
Electron close 事件 → executeJavaScript 强制写入
持久化键名: private-novel-studio-pro-storage
数据迁移: loadProject() 兼容旧版本字段（自动补充缺失字段默认值）
Electron 主进程持久化实现
在 src/main/index.ts 中，窗口关闭时执行：
TypeScript
复制
mainWindow.on('close', () => {
  mainWindow.webContents.executeJavaScript(`
    window.dispatchEvent(new Event('beforeunload'));
    localStorage.setItem('force-save', Date.now().toString());
  `);
});
工具函数
...
linux.target	AppImage	Linux 安装包格式
...
│   │   └── preload.ts          # preload 桥接脚本（输出为 preload.mjs）
│   ├── config/
