/**
 * 感情线交互增强组件
 * SRS v2.3 中优先级：支持拖拽节点、添加/删除节点、修改类型
 */

import React, { useState, useCallback } from 'react'
import ReactFlow, {
  Background, Controls, MiniMap, useNodesState, useEdgesState,
  Node, Edge, addEdge, Connection, ReactFlowProvider,
} from 'reactflow'
import 'reactflow/dist/style.css'
import { useStore } from '../../store'
import type { EmotionArcNode, EmotionArcEdge, EmotionEventType } from '../../../config/types'
...
export default function EmotionFlowEditor() {
  ...
  const initialNodes: Node[] = (emotionArc?.nodes || []).map((n) => ({
    id: n.id,
    type: 'default',
    position: n.position,
  ...
  const initialEdges: Edge[] = (emotionArc?.edges || []).map((e) => ({
    id: e.id,
    source: e.source,
    target: e.target,
    type: e.type,
  ...
  const handleSaveNode = () => {
    ...
    const updatedNodes = emotionArc.nodes.map((n) =>
      n.id === selectedNode
        ? {
            ...n,
            data: {
    ...
    updateEmotionArc({ nodes: updatedNodes })
    setShowNodeEditor(false)
  }

  const handleAddNode = () => {
    if (!emotionArc) return
    const newNode: EmotionArcNode = {
      id: `node_${Date.now()}`,
      type: 'emotion',
    ...
    updateEmotionArc({ nodes: [...emotionArc.nodes, newNode] })
  }
  ...
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{
        ...
        <button onClick={handleAddNode} style={{ ...btnStyle, background: '#8b5cf6', color: '#fff' }}>
          ➕ 添加节点
        </button>
        {selectedNode && (
          <>
            <button onClick={() => setShowNodeEditor(true)} style={btnStyle}>
              ✏️ 编辑
            </button>
            <button onClick={handleDeleteNode} style={{ ...btnStyle, background: '#ef4444', color: '#fff' }}>
              🗑️ 删除
            </button>
          </>
        )}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              ...
              <div>
                ...
                <input
                  type="range"
                  min={0} max={100}
                  value={editingNode.data.intensity || 50}
